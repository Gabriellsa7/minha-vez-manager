...
import { useGetExamAvailabilityRules } from '../../config/api/get-exam-availability-rules';
import { usePutExamAvailabilityRules } from '../../config/api/put-exam-availability-rules';
import { useGetExamAvailabilityBlackouts } from '../../config/api/get-exam-availability-blackouts';
        ...
        <section className={style.rulesSection}>
          ...
          {rows.map((row) => (
            <div key={row.weekday} className={style.ruleRow}>
              ...
              <div className={style.fieldGroup}>
                <span className={style.fieldLabel}>Duração da vaga (min)</span>
                <input
                  type="number"
                  min="1"
                  disabled={!row.enabled}
          ...
          <div className={style.blackoutList}>
            {blackouts?.length ? (
              ...
              <EmptyState
                ...
                description="Datas bloqueadas não recebem agendamentos de exame."
              />
            )}
          </div>
        </section>
